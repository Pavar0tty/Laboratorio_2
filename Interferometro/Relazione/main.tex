...
di $\Delta d$ e quindi calibrare il micrometro confrontando la misura ottenuta con il nonio rispetto a quella dell'interferometro.\\
...
e il coseno dell'angolo $\theta$ approssimato a circa 1, assumendo incidenza normale.
...
Abbiamo eseguito la misura sei volte, variando per quanto possibile sempre di $20\ \text{\SIUnitSymbolMicro m}$ la distanza tra i due specchi.
Per la stima della distanza, riportiamo il valor medio e l'errore standard risultanti:
$$\Delta d_\text{mis} = (19.41 \pm0.11)\ \text{\SIUnitSymbolMicro m} $$
Per questi calcoli abbiamo utilizzato come valore tabulato la lunghezza d'onda del laser $\lambda_{laser} = \SI{632.8}{\nano\meter}$.
Abbiamo inoltre verificato che utilizzando $\lambda_\text{aria} = \frac{\lambda_0}{n_\text{aria}}$ dove $n_\text{aria} = 1,0003$,
$\Delta d_\text{mis} \approx 19.40\ \text{\SIUnitSymbolMicro m}$, una variazione trascurabile.
...
il valore di $\Delta d$ fosse diverso, nonostante fosse vicino a 20 \text{\SIUnitSymbolMicro m}. Ripetendo più volte
la misura e calcolando la media, ci siamo avvicinati il più possibile ad ottenere una stima della distanza di tipo 
gaussiano, dominata da errore normale. \\
...
Ci siamo serviti della formula \eqref{eq:indice_rifrazione_aria} per risalire all'indice di rifrazione dell'aria :
\begin{align}
    n &= 1 + m \cdot P \\
...
il valore di $m$ ottenuto è:
$$m = (2.484 \pm 0.033)\cdot 10^{-9} \ \text{Pa}^{-1}$$
...
$$ \lambda_\text{mis}= (640 \pm 19)\ \text{nm}$$

Dopo aver eseguito un test di compatibilità con $\lambda_{laser} = \SI{632.8}{\nano\meter}$ otteniamo $t \approx 0.32$, che corrisponde ad una percentuale
di compatibilità di circa il 35\% $\gg$ 5\%.\\
