...
\begin{table}[h!]
    ...
    \hline
    \textbf{Misura} & \textbf{Frange $\Delta N$} & \textbf{Distanza $\Delta d$}  \\
    \hline
    1 & 61 & 19.300 \\
    2 & 64 & 20.249 \\
    3 & 59 & 18.667 \\
...
Riportiamo in seguito per la stima della distanza il valor medio e la deviazione standard $$\Delta d_\text{mis} = 19.41 \pm0.07\text{\SIUnitSymbolMicro m} $$riportiamo  inoltre
...
la misura per quattro volte, spostando conm un passo di $20 \text{\SIUnitSymbolMicro m}$, (valore segnato 
...
misure non dotate di errore, perchè ricavate a partire da N e $\lambda$:
$$\Delta d_\text{mis} = 19.4 \pm0.05\text{\SIUnitSymbolMicro m} $$
...
Inserire qualcosa su quale dei due metodi quindi ha dato un risultato più preciso (Michelson)
\newpage

\subsection{Misura indice rifrazione aria}
In questa parte dell'esperienza abbiamo utilizzato l'interferometro in configurazione Michelson per misurare l'indice di rifrazione 
dell'aria, sfruttando il fatto che, variando la pressione cambia l'indice e di conseguenza il numero di frange di interferenza che scorrono. La configurazione
ricalca quella della figura \ref{fig:michelson config}, con la differenza che abbiamo inserito una cella a vuoto in uno 
dei bracci dell'interferometro.\\
Abbiamo utilizzata la formula \ref{eq:indice_rifrazione_aria} per risalire all'indice di rifrazione dell'aria :
\begin{equation}
...
la miglior stima del valore ottenuto per l'indice di rifrazione dell'aria, con relativo errore:
 $$n_{aria} = 1.00035 \pm 0.00002$$\\
...
Operando un test di compatibilità con il valore atteso per l'indice di rifrazione dell'aria, pari a $n_{aria} = 1.00029$, otteniamo una distanza in 
deviazioni standard pari a 2.37.\\

\subsection{Misura indice rifrazione vetro}
Sempre mantenendo la configuazione Michelson, abbiamo montato un supporto con una lastra di vetro
per cercare di ricavarne l'indice di rifrazione. Servendoci di un rotational pointer è possibile
...
%Non c'è errore perchè sul numero di frange non c'è errore
